...
#define LOG_XXX2(func, ec) LOG_INFO("[" func "] sid: " << sid << " current:" << current_ << " next:" << next_ << " ec:" << ec.message());
            ...
            response_t const & resp)
        {
            ...
            if (main_ses) {
                // 主会话已经存在，不需要做什么
                if (next_ == NULL || main_ses != next_->next()) { // 前没有会话组或者不是会话组的下一个会话
                    ec = error::session_kick_out;
                } else {
                }
            } else if (current_ == NULL) { // 当前没有会话组
                current_ = next_ = create_group(url, ec);
            } else if (next_ != current_) { // 有“当前会话组”和“等待会话组”
                if (!next_->accept(url)) {
            ...
            } else if (!next_->accept(url)) { // 只有“当前会话组”
                SessionGroup * group = create_group(url, ec);
                if (group) {
                    if (current_->busy()) {
                        if (!canceling_) {
